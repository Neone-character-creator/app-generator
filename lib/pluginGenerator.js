...
const singleModelGenerator = require("./model/modelClassSourceGenerator");

const duplicateProjectTemplateDirectory = require('./duplicateProjectTemplateDirectory');
...
function PluginGenerator(configuration, tmpDir) {
    ...
    this.tmpDir = tmpDir;
    this.modelClassGenerator = new ModelClassGenerator(singleModelGenerator, configuration.model, tmpDir);
    this.modelClassWriter = ModelClassWriter;
}

PluginGenerator.prototype.duplicateProjectDirectory = duplicateProjectTemplateDirectory;
...
PluginGenerator.prototype.generateComponents = require("./componentGenerator");
...
PluginGenerator.prototype.copyToTargetDirectory = (configuration, projectName, tmpDir) => {
    fs.copy(tmpDir, path.resolve(configuration.rootDir, `buildPlugin`));
};
...
PluginGenerator.prototype.copyFields = function(configuration, tmpDir) {
    if (configuration.fieldsPath) {
        var fieldsPath = path.resolve(configuration.rootDir, configuration.fieldsPath);
        if (!fs.existsSync(fieldsPath)) {
            throw new Error("Fields configuration in location " + fieldsPath + " does not exist");
        }
        var copyTo = path.resolve(tmpDir, "src", "main", "resources", "fields.json");
        fs.copyFileSync(fieldsPath, copyTo);
    }
};
PluginGenerator.prototype.generate = async function () {
    const projectName = this.configuration.alphanumericGameName;
    return this.duplicateProjectDirectory(projectName, this.tmpDir)
        ...
        .then(invokeWithInput(this.generateComponents.bind(this, this.configuration), {as: "generatedComponents"}))
        ...
        .then(invokeWithInput(this.copyFields.bind(this, this.configuration), false, "tmpDir"))
