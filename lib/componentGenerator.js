...
module.exports = async function (applicationConfiguration) {
    ...
    views.forEach(view => {
        components.views[view] = {
            path: `views/${view}.js`,
            content: babel.transformSync(generateViewComponent(view)(applicationConfiguration)).code
        }
    });
    ...
    Object.keys(applicationConfiguration.components).forEach(componentName => {
        components.components[componentName] = {
            ...
            content: babel.transformSync(generateComponent(applicationConfiguration.components[componentName].type)(componentName)(applicationConfiguration)).code
        }
    });
