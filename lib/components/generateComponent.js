...
function replaceViews(config, generatedContent) {
    ...
    const replacementText = leadingWhitespace ? Object.keys(config.views).map(viewName => {
...
function defaultReplaceChildren(componentName, appConfig, generatedContent) {
    ...
    const replacementText = (appConfig.components[componentName].children || []).map(childName => {
        ...
        switch (childConfig.type) {
            case "view":
                return `<${childName}View />`;
            case "textfield":
                return `<TextField id="${childName}" value="" />`;
            case "container":
                return `<Grid id="${childName}" direction="${childConfig.direction === 'vertical' ? 'column' : 'row'}" />`
            default:
                throw new Error(`${childConfig.type} type on component ${childName} is invalid`);
        }
    }).join('\n');
