...
function replaceAppImports(config, generatedContent) {
    const injectedImports = config.views.map(viewName => {
        return `import ${camelCaseViewName(viewName)} from "../components/${camelCaseViewName(viewName)}"`
    }).join('\n');
        ...
        switch (childConfig.type) {
            ...
            case "container":
                if (!imports[childName]) {
                    ...
                    return `import ${pascalCaseName(childName)}Container from './${pascalCaseName(childName)}Container';`
                }
            // default:
...
function defaultReplaceChildren(componentName, appConfig, generatedContent) {
    ...
    const replacementText = (appConfig.components[componentName].children || []).map(childName => {
        ...
        switch (childConfig.type) {
            case "textfield":
                return `<TextField label="${childConfig.label}" id="${childName}" value="" />`;
            case "container":
                return `<${pascalCaseName(childName)}Container />`;
        }
    }).join('\n');
