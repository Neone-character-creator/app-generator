...
describe("Transformer generator", function () {
    ...
    it("generates a transformer to override a value for a SET", function () {
        const transformers = transformerCalculator(state, "SET", "", 0);
    ...
    it("replace a SET transformer with a new one for the same path", function () {
        state.transformers = transformerCalculator(state, "SET", "", 0);
        state.transformers = transformerCalculator(state, "SET", "", 1);
    ...
    it("SET for different paths exist", function () {
        state.transformers = transformerCalculator(state, "SET", "a", 0);
        state.transformers = transformerCalculator(state, "SET", "b", 1);
        expect(state.transformers).toEqual([{
            path: "a",
            action: "SET",
            value: 0
    ...
    it("generates a transformer to insert a value for PUSH", function () {
        ...
        state.transformers = transformerCalculator(state, "PUSH", "a", 0);
        expect(state.transformers).toEqual([{
            path: "a",
            action: "PUSH",
            value: 0,
    ...
    it("REMOVE undoes a PUSH at the same path", function () {
        state.transformers = transformerCalculator(state, "PUSH", "a", 1);
        state.transformers = transformerCalculator(state, "REMOVE", "a", 0)
        expect(state.transformers).toEqual([]);
    });
    it("generates a transformer to increase a value with an ADD", function(){
        state.transformers = transformerCalculator(state, "ADD", "a", 1);
    ...
    it("generates a transformer to reduce a value with an ADD", function(){
        state.transformers = transformerCalculator(state, "SUBTRACT", "a", 1);
        expect(state.transformers[0]).toEqual({
            path: "a",
            action: "SUBTRACT",
            value: 1
    ...
    describe("value requirements", function () {
        it("are added to the transfomer created to add them", function () {
            state.a = 1;
            state.transformers = transformerCalculator(state, "ADD", "b", {
                requires: [
                    {
                        path: "a",
                        comparison: "equals",
