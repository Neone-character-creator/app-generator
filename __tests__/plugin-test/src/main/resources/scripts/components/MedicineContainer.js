...
import MedicineLabelLabel from './MedicineLabelLabel';
import MedicineCareerFlagCheckbox from './MedicineCareerFlagCheckbox';
import MedicineRatingNumber from './MedicineRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in medicine",
            hasError: true
        };
    }
    ...
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="medicine" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<MedicineLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<MedicineCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<MedicineRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
