...
import MeleeLabelLabel from './MeleeLabelLabel';
import MeleeCareerFlagCheckbox from './MeleeCareerFlagCheckbox';
import MeleeRatingNumber from './MeleeRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in melee",
            hasError: true
        };
    }
    ...
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="melee" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<MeleeLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<MeleeCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<MeleeRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
