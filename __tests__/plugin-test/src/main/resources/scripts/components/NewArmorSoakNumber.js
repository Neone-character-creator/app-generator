...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in newArmorSoak",
            hasError: true
        };
    }
...
class newArmorSoak extends React.Component {
    constructor(props){
            super(props);
    ...
    render() {
        ...
        return (
        <ErrorBoundary>
            <Textfield id="newArmorSoak" label="Soak" type="number" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const Component = newArmorSoak;
const isBoundComponent = true
const boundPropertyExpression = "$state.selectedArmor.soak"; // The expression used to bind this component to the state.
