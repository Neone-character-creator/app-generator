...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in addArmor",
            hasError: true
        };
    }
...
class addArmor extends React.Component {
    constructor (props){
        super(props);
        this.click = function(){
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="addArmor" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const action = "# {type:'ADD', path:'character.armor', value:$state.selectedArmor}";
const disabledWhenExpression = "# !_.get($state, 'selectedArmor')";
