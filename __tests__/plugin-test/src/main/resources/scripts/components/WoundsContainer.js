...
import ThresholdWoundsNumber from './ThresholdWoundsNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in wounds",
            hasError: true
        };
    }
    ...
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="wounds" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={12}>
<ThresholdWoundsNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
