...
import ObligationTypeTextfield from './ObligationTypeTextfield';
import ObligationDescriptionTextfield from './ObligationDescriptionTextfield';
import ObligationRatingNumber from './ObligationRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in obligationItem",
            hasError: true
        };
    }
...
class obligationItem extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="obligationItem" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<ObligationTypeTextfield value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<ObligationDescriptionTextfield value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<ObligationRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
...
const Component = obligationItem;
