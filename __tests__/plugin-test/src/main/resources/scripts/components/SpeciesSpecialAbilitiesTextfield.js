...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in speciesSpecialAbilities",
            hasError: true
        };
    }
...
class Component extends React.Component {
    ...
    render(){
        ...
        return (
        <ErrorBoundary>
        <Textfield disabled={this.props.disabled} id="speciesSpecialAbilities" fullWidth={true} label="Special Abilities" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const valuePropertyExpression = "# $state.character.species.specialAbilities.map(sa => sa.name).join(', ') "; // The expression evaluated to determine the value of the component
