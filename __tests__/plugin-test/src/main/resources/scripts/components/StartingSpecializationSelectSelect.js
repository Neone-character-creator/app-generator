...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in startingSpecializationSelect",
            hasError: true
        };
    }
...
class startingSpecializationSelect extends React.Component {
    constructor(props){
        super(props);
    ...
    render(){
        ...
        <FormControl fullWidth={true}>
                    <InputLabel id="startingSpecializationSelect-label"></InputLabel>
                    <Select id="startingSpecializationSelect" autoWidth={true} value={this.props.value} onChange={this.update}
...
const Component = startingSpecializationSelect;
const listItemDisabledExpression = JSON.parse(_.unescape("null"));
const isBoundComponent = true
const boundPropertyExpression = "$state.character.specializations[0]"; // The expression used to bind this component to the state.
...
const optionsExpression = "# ($state.character.career.specializations || []).map(sn => $model.specialization.values.find(s => s.name === sn)).concat([$model.specialization.values.find(s => s.name === 'Force Sensitive Exile')])";
