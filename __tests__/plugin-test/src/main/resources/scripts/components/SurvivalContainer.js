...
import SurvivalLabelLabel from './SurvivalLabelLabel';
import SurvivalCareerFlagCheckbox from './SurvivalCareerFlagCheckbox';
import SurvivalRatingNumber from './SurvivalRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in survival",
            hasError: true
        };
    }
    ...
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="survival" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<SurvivalLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<SurvivalCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<SurvivalRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
