...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in soak",
            hasError: true
        };
    }
...
class soak extends React.Component {
    constructor(props){
            super(props);
    ...
    render() {
        ...
        return (
        <ErrorBoundary>
            <Textfield id="soak" label="Soak Value" type="number" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const Component = soak;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.soak"; // The expression used to bind this component to the state.
