...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in startingSpecializationTalentCost",
            hasError: true
        };
    }
...
class startingSpecializationTalentCost extends React.Component {
    constructor(props){
            super(props);
    ...
    render() {
        ...
        return (
        <ErrorBoundary>
            <Textfield id="startingSpecializationTalentCost" label="cost" type="number" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const Component = startingSpecializationTalentCost;
...
const valuePropertyExpression = "# $this.tier * 5"; // The expression evaluated to determine the value of the component
