...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in resilienceCareerFlag",
            hasError: true
        };
    }
...
class resilienceCareerFlag extends React.Component {
    constructor(props){
        super(props);
    ...
    render() {
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Checkbox id="resilienceCareerFlag" checked={this.props.value} onChange={this.update} />
            </ErrorBoundary>)
        } else {
            return null
        }
...
const Component = resilienceCareerFlag;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.resilience.isCareerSkill"; // The expression used to bind this component to the state.
