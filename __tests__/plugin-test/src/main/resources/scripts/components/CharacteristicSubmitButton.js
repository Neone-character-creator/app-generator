...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in characteristicSubmit",
            hasError: true
        };
    }
...
class characteristicSubmit extends React.Component {
    constructor (props){
        super(props);
        this.click = function(){
            this.props.dispatch(this.props.clickAction);
        }
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="characteristicSubmit" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const Component = characteristicSubmit;
...
const valuePropertyExpression = "# 'Buy'"; // The expression evaluated to determine the value of the component
const readOnly = true; // If this component only calculates its value, or is bound and can update it.
const optionsExpression = "";
const action = "# {type:'ADVANCEMENT', advancementType:'characteristic', advancement: _.get($state, 'selectedAdvancement.characteristic.option')}";
const disabledWhenExpression = "# _.get($state, 'selectedAdvancement.characteristic.isAvailable') !== true";
