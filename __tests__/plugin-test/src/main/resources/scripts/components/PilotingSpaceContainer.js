...
import PilotingSpaceLabelLabel from './PilotingSpaceLabelLabel';
import PilotingSpaceCareerFlagCheckbox from './PilotingSpaceCareerFlagCheckbox';
import PilotingSpaceRatingNumber from './PilotingSpaceRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in pilotingSpace",
            hasError: true
        };
    }
...
class pilotingSpace extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="pilotingSpace" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<PilotingSpaceLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<PilotingSpaceCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<PilotingSpaceRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
