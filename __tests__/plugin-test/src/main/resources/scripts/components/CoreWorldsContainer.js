...
import CoreWorldsLabelLabel from './CoreWorldsLabelLabel';
import CoreWorldsCareerFlagCheckbox from './CoreWorldsCareerFlagCheckbox';
import CoreWorldsRatingNumber from './CoreWorldsRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in coreWorlds",
            hasError: true
        };
    }
...
class coreWorlds extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="coreWorlds" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<CoreWorldsLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<CoreWorldsCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<CoreWorldsRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
