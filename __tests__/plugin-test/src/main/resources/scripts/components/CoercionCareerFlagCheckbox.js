...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in coercionCareerFlag",
            hasError: true
        };
    }
...
class coercionCareerFlag extends React.Component {
    constructor(props){
        super(props);
    ...
    render() {
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Checkbox id="coercionCareerFlag" checked={this.props.value} onChange={this.update} />
            </ErrorBoundary>)
        } else {
            return null
        }
...
const Component = coercionCareerFlag;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.coercion.isCareerSkill"; // The expression used to bind this component to the state.
