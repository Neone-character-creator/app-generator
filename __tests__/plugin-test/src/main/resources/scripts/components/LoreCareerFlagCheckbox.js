...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in loreCareerFlag",
            hasError: true
        };
    }
...
class loreCareerFlag extends React.Component {
    constructor(props){
        super(props);
    ...
    render() {
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Checkbox id="loreCareerFlag" checked={this.props.value} onChange={this.update} />
            </ErrorBoundary>)
        } else {
            return null
        }
...
const Component = loreCareerFlag;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.knowledgeLore.isCareerSkill"; // The expression used to bind this component to the state.
