...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in pilotingSpaceCareerFlag",
            hasError: true
        };
    }
...
class pilotingSpaceCareerFlag extends React.Component {
    constructor(props){
        super(props);
    ...
    render() {
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Checkbox id="pilotingSpaceCareerFlag" checked={this.props.value} onChange={this.update} />
            </ErrorBoundary>)
        } else {
            return null
        }
...
const Component = pilotingSpaceCareerFlag;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.pilotingSpace.isCareerSkill"; // The expression used to bind this component to the state.
