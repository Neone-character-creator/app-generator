...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in singlePurchasedAdvancementRemoveButton",
            hasError: true
        };
    }
...
class singlePurchasedAdvancementRemoveButton extends React.Component {
    constructor (props){
        super(props);
        this.click = function(){
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="singlePurchasedAdvancementRemoveButton" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const Component = singlePurchasedAdvancementRemoveButton;
...
const valuePropertyExpression = "# `Remove`"; // The expression evaluated to determine the value of the component
const readOnly = true; // If this component only calculates its value, or is bound and can update it.
const optionsExpression = "";
const action = "# `REMOVE-ADVANCEMENT ${$this.value}`";
