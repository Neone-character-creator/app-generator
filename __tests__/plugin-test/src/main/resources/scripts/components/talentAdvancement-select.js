...
const boundProperty = "$state.selectedAdvancement.talent"; // The expression used to determine the value of binding.
const readOnly = false; // If this component only calculates its value, or is bound and can update it.
const valueExpression = "# $state.availableAdvancements.talent.options.map(o => _.cloneDeep(o)).map(o => {o.option.value = $model.talent.values.find(t => t.id === o.option.value);return o;})";
