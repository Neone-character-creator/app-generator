...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in otherCareerFlag",
            hasError: true
        };
    }
...
class otherCareerFlag extends React.Component {
    constructor(props){
        super(props);
    ...
    render() {
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Checkbox id="otherCareerFlag" checked={this.props.value} onChange={this.update} />
            </ErrorBoundary>)
        } else {
            return null
        }
...
const Component = otherCareerFlag;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.knowledgeOther.isCareerSkill"; // The expression used to bind this component to the state.
