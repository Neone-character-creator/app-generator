...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in newOtherItem",
            hasError: true
        };
    }
...
class newOtherItem extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="newOtherItem" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={1}>
<AddOtherItemButton value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={11}>
<NewOtherItemTextfield value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
