...
import AvailableExperienceTextfield from './AvailableExperienceTextfield';
import TotalExperienceNumber from './TotalExperienceNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in experience",
            hasError: true
        };
    }
    ...
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="experience" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={6}>
<AvailableExperienceTextfield value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={6}>
<TotalExperienceNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
