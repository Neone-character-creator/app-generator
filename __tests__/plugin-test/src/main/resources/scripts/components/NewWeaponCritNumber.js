...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in newWeaponCrit",
            hasError: true
        };
    }
...
class newWeaponCrit extends React.Component {
    constructor(props){
            super(props);
    ...
    render() {
        ...
        return (
        <ErrorBoundary>
            <Textfield id="newWeaponCrit" label="Crit" type="number" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const Component = newWeaponCrit;
const isBoundComponent = true
const boundPropertyExpression = "$state.selectedWeapon.criticalRating"; // The expression used to bind this component to the state.
