...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in addOtherItem",
            hasError: true
        };
    }
...
class addOtherItem extends React.Component {
    constructor (props){
        super(props);
        this.click = function(){
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="addOtherItem" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const action = "# {type:'ADD', path: 'character.otherItems', value: $state.newItem }";
