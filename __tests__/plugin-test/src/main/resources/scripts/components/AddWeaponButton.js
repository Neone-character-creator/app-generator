...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in addWeapon",
            hasError: true
        };
    }
...
class addWeapon extends React.Component {
    constructor (props){
        super(props);
        this.click = function(){
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="addWeapon" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const valuePropertyExpression = "# 'Add'"; // The expression evaluated to determine the value of the component
const readOnly = true; // If this component only calculates its value, or is bound and can update it.
const optionsExpression = "";
const action = "# {type:'ADD', path:'character.weapons', value:$state.selectedWeapon}";
const disabledWhenExpression = "# !_.get($state, 'selectedWeapon')";
