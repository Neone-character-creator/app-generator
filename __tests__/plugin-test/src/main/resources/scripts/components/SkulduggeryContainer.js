...
import SkulduggeryLabelLabel from './SkulduggeryLabelLabel';
import SkulduggeryCareerFlagCheckbox from './SkulduggeryCareerFlagCheckbox';
import SkulduggeryRatingNumber from './SkulduggeryRatingNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in skulduggery",
            hasError: true
        };
    }
...
class skulduggery extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="skulduggery" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<SkulduggeryLabelLabel value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<SkulduggeryCareerFlagCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<SkulduggeryRatingNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
