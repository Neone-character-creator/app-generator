...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in loreRating",
            hasError: true
        };
    }
...
class loreRating extends React.Component {
    constructor(props){
            super(props);
    ...
    render() {
        ...
        return (
        <ErrorBoundary>
            <Textfield id="loreRating" label="Rating" type="number" value={this.props.value} onChange={this.update} />
        </ErrorBoundary>
        );
        } else {
            return null
...
const Component = loreRating;
const isBoundComponent = true
const boundPropertyExpression = "$state.character.skills.knowledgeLore.rank"; // The expression used to bind this component to the state.
