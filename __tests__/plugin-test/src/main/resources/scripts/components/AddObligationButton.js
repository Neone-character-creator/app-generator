...
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in addObligation",
            hasError: true
        };
    }
...
class addObligation extends React.Component {
    ...
    render() {
        ...
        return (
            <ErrorBoundary>
        <Button id="addObligation" variant="contained" onClick={this.click} disabled={this.props.disabled} >
                    {this.props.value}
                </Button>
                </ErrorBoundary>);
        } else {
...
const action = "# {type:'ADD', path:'character.obligations', value: _.get($state, 'newObligation')}";
