...
import StartingSpecializationTalentIsBoughtCheckbox from './StartingSpecializationTalentIsBoughtCheckbox';
import StartingSpecializationTalentNameTextfield from './StartingSpecializationTalentNameTextfield';
import StartingSpecializationTalentCostNumber from './StartingSpecializationTalentCostNumber';
class ErrorBoundary extends React.Component {
    ...
    static getDerivedStateFromError(error){
        return {
            error: "There was an error in startingSpecializationAvailableTalentsWrapper",
            hasError: true
        };
    }
...
class startingSpecializationAvailableTalentsWrapper extends React.Component {
    render(){
        if(!this.props.hidden){
            return (<ErrorBoundary>
                <Grid id="startingSpecializationAvailableTalentsWrapper" style={defaultStyle} container spacing={8} justify="space-around" direction={"horizontal" == "vertical" ? "column" : "row"}>
                    <Grid item xs={4}>
<StartingSpecializationTalentIsBoughtCheckbox value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<StartingSpecializationTalentNameTextfield value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
<Grid item xs={4}>
<StartingSpecializationTalentCostNumber value={this.props.value} index={this.props.index} parent={this.props}/>
</Grid>
                </Grid>
            </ErrorBoundary>);
        } else {
...
const Component = startingSpecializationAvailableTalentsWrapper;
